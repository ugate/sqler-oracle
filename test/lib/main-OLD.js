...
class Tester {
   ...
   */
  static async before() {
    test.ci = 'CI' in process.env;
    Labrat.header(`Creating test tables ${test.ci ? `(CI=${test.ci})` : ''}`);
    
    const conf = getConf();
    test.cache = null;
    test.mgr = new Manager(conf, test.cache, test.mgrLogit);
    await test.mgr.init();
    
    await test.mgr.db.tst.ora_test.create.tables();
    test.created = true;
  }
  ...
  static async after() {
    ...
    if (test.ci) { // drop isn't really need in CI env
      try {
        await test.mgr.db.tst.ora_test.delete.tables();
        test.created = false;
      } catch (err) {
        if (LOGGER.warn) LOGGER.warn(`Failed to delete tables (CI=${test.ci})`, err);
      }
    } else {
      await test.mgr.db.tst.ora_test.delete.tables();
      test.created = false;
  ...
  static async confHostMissing() {
    ...
    new Manager(conf, test.cache, test.mgrLogit);
  }
  ...
  static async confCredentialsInvalid() {
    ...
    const mgr = new Manager(conf, test.cache, false);
    await mgr.init();
    try {
      await mgr.close();
    } catch (err) {
  ...
  static async create() {
    ...
    const txId = await test.mgr.db.tst.beginTransaction();
    const prom = insertLob(test.lobFile, txId);
...
function getConf(noPool) {
  ...
  const conf = {
    ...
    "univ": {
      "db": {
        "testId": JSON.parse(JSON.stringify(creds))
      }
    },
    "db": {
      "dialects": {
 ...
 * @param {SQLERExecOptions} [opts] The `sqler` execution options
...
async function rows(op, opts, testOpts) {
  ...
  if (LOGGER.info) LOGGER.info(`Performing "${op}" on ${test.rowCount} test records`);

  // default autoCommit = true
  const autoCommit = opts && opts.hasOwnProperty('autoCommit') ? opts.autoCommit : true;

  if (!test.mgr) {
    await crudManager(testOpts && testOpts.hasOwnProperty('managerLogger') ? testOpts.managerLogger : test.mgrLogit);
  }

  let txId;
  if (!autoCommit) {
    txId = await test.mgr.db.tst.beginTransaction();
  }
  ...
  if (op === 'read-all') {
    ...
    proms = [ test.mgr.db.tst.auxy.all.rows(opts) ];
  } else {
    rowCount = 1; // one row at a time
    proms = new Array(test.rowCount);
    const date = new Date();
    for (let i = 0, xopts; i < test.rowCount; i++) {
      ...
      proms[i] = test.mgr.db.tst[op].table.rows(xopts);
    }
  }
 ...
 * @returns {SQLERExecResults} The LOB results
 */
async function insertLob(lobFile, txId) {
  ...
  const rslt = await test.mgr.db.tst.create.table2.rows(xopts);
  if (!txId) return rslt;
...
async function readLob(txId) {
  ...
  const rslt = await test.mgr.db.tst.read.table2.rows(xopts);
...
async function crudManager(managerLogger) {
  ...
  test.mgr = new Manager(conf, test.cache, managerLogger || test.mgrLogit);
  return test.mgr.init();
}
